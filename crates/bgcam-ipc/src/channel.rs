...
impl ObjectNames {
    pub fn with_prefix(prefix: &str) -> Self {
        Self {
            section: format!("{prefix}{BGCAM_SECTION_NAME}"),
            frame_ready: format!("{prefix}{BGCAM_FRAME_READY_EVENT_NAME}"),
            consumer_changed: format!("{prefix}{BGCAM_CONSUMER_CHANGED_EVENT_NAME}"),
        }
    }
...
fn map(section: &OwnedHandle) -> Result<(MappedView, SharedRegion), IpcError> {
    ...
    let region = unsafe { SharedRegion::from_raw(view.0.Value.cast::<u8>(), BGCAM_SECTION_SIZE as usize) }?;
    Ok((view, region))
}
    ...
    pub fn create(names: &ObjectNames) -> Result<Self, IpcError> {
        ...
        unsafe {
            ConvertStringSecurityDescriptorToSecurityDescriptorW(
                &HSTRING::from(BGCAM_OBJECT_SDDL),
                SDDL_REVISION_1,
                &mut descriptor,
                None,
            )
