...
impl PixelFormat {
    pub fn code(self) -> u32 {
        match self {
            Self::Nv12 => BGCAM_FORMAT_NV12,
            Self::Bgra => BGCAM_FORMAT_BGRA,
        }
    }

    pub fn from_code(code: u32) -> Option<Self> {
        match code {
            BGCAM_FORMAT_NV12 => Some(Self::Nv12),
            BGCAM_FORMAT_BGRA => Some(Self::Bgra),
            _ => None,
        }
    }
...
pub struct SharedRegion {
    header: NonNull<BgcamFrameHeader>,
    data: NonNull<u8>,
    capacity: usize,
}
...
fn validate_size(width: u32, height: u32) -> Result<(), IpcError> {
    let valid = width > 0
        ...
        && width <= BGCAM_MAX_WIDTH
        && height <= BGCAM_MAX_HEIGHT;
    if valid {
        Ok(())
    } else {
...
impl SharedRegion {
    ...
    pub unsafe fn from_raw(base: *mut u8, len: usize) -> Result<Self, IpcError> {
        let required = BGCAM_HEADER_SIZE as usize + 1;
        if len < required {
            return Err(IpcError::RegionTooSmall { actual: len, required });
        }
        let header =
            NonNull::new(base.cast::<BgcamFrameHeader>()).ok_or(IpcError::RegionTooSmall { actual: 0, required })?;
        let data = NonNull::new(unsafe { base.add(BGCAM_HEADER_SIZE as usize) })
            .ok_or(IpcError::RegionTooSmall { actual: 0, required })?;
        Ok(Self {
            header,
            data,
            capacity: len - BGCAM_HEADER_SIZE as usize,
        })
    }

    fn raw(&self) -> *mut BgcamFrameHeader {
        self.header.as_ptr()
    }
    ...
    pub fn initialize(&self, qpc_frequency: i64) {
        let header = self.raw();
        unsafe {
            addr_of_mut!((*header).magic).write_volatile(BGCAM_MAGIC);
            addr_of_mut!((*header).version).write_volatile(BGCAM_PROTOCOL_VERSION);
            addr_of_mut!((*header).header_size).write_volatile(BGCAM_HEADER_SIZE);
    ...
    pub fn validate(&self) -> Result<(), IpcError> {
        ...
        if magic == BGCAM_MAGIC && version == BGCAM_PROTOCOL_VERSION {
            Ok(())
        } else {
            Err(IpcError::IncompatibleProtocol {
                magic,
                version,
                expected_magic: BGCAM_MAGIC,
                expected_version: BGCAM_PROTOCOL_VERSION,
            })
        }
    }
    ...
    pub fn read_frame(&self, out: &mut [u8]) -> Result<Option<FrameInfo>, IpcError> {
        let header = self.raw();
        let sequence = self.sequence();
        for _ in 0..BGCAM_SEQLOCK_ATTEMPTS {
    ...
    pub fn set_producer_active(&self, active: bool, qpc: i64) {
        ...
        if active {
            flags.fetch_or(BGCAM_PRODUCER_ACTIVE, Ordering::AcqRel);
        } else {
            flags.fetch_and(!BGCAM_PRODUCER_ACTIVE, Ordering::AcqRel);
        }
        self.atomic_i64(unsafe { addr_of_mut!((*header).producer_heartbeat_qpc) })
            .store(qpc, Ordering::Relaxed);
    }
    ...
    pub fn producer_alive(&self, now_qpc: i64) -> bool {
        ...
        flags & BGCAM_PRODUCER_ACTIVE != 0 && self.fresh(heartbeat, now_qpc)
    }

    fn fresh(&self, heartbeat: i64, now_qpc: i64) -> bool {
        let frequency = unsafe { addr_of!((*self.raw()).qpc_frequency).read_volatile() }.max(1);
        let timeout = frequency * i64::from(BGCAM_HEARTBEAT_TIMEOUT_MS) / 1000;
    ...
    pub fn consumer_started(&self, format: PixelFormat, qpc: i64) {
        ...
        self.atomic_u32(unsafe { addr_of_mut!((*header).consumer_flags) })
            .fetch_or(BGCAM_CONSUMER_ACTIVE, Ordering::AcqRel);
    ...
    pub fn consumer_stopped(&self) {
        ...
        if previous <= 1 {
            self.atomic_u32(unsafe { addr_of_mut!((*header).consumer_flags) })
                .fetch_and(!BGCAM_CONSUMER_ACTIVE, Ordering::AcqRel);
        }
    }
    ...
    pub fn consumer_state(&self, now_qpc: i64) -> ConsumerState {
        ...
        let active = flags & BGCAM_CONSUMER_ACTIVE != 0 && count > 0 && self.fresh(heartbeat, now_qpc);
        ConsumerState {
            active,
...
mod tests {
    ...
    fn region(capacity: usize) -> (Vec<u64>, SharedRegion) {
        let words = (BGCAM_HEADER_SIZE as usize + capacity).div_ceil(8);
