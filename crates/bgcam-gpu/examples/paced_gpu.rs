...
use bgcam_core::{BackgroundEffect, ColorMatrix, FrameSize, OutputFormat, PipelineSettings, Rgb, RgbImage};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let models = std::env::var_os("BGCAM_MODELS_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../models"));
    let photo_path = std::env::var_os("BGCAM_PERSON_IMAGE").ok_or("set BGCAM_PERSON_IMAGE")?;
