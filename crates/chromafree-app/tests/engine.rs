...
use chromafree_app::camera::{CameraOpenError, CameraProvider, OpenedCamera};
...
fn engine_follows_consumers_formats_preview_and_settings() {
    let names = ObjectNames::with_prefix(&format!("Local\\chromafree-engine-test-{}-", std::process::id()));
    let counters = Arc::new(Counters::default());
    let recorder = Recorder::default();
    let engine = Engine::start(
        EngineOptions {
            names: names.clone(),
            models_dir: std::env::temp_dir().join("chromafree-no-models"),
