...
use bgcam_capture::{CameraDevice, CaptureFormat, MediaFoundation, camera_formats, list_cameras};
use bgcam_core::{BgraFrame, ModelVariant, available_rvm_variants};
use bgcam_ipc::{ObjectNames, PixelFormat};
use slint::{
...
pub fn status_lines(status: &EngineStatus) -> (String, String, String, bool) {
    let headline = match &status.state {
        EngineState::Idle => "Czeka na aplikację, która użyje kamery bgcam".to_owned(),
    ...
    let virtual_camera = match &status.virtual_camera {
        ...
        VirtualCameraState::Registered => "Wirtualna kamera „bgcam” jest dostępna w innych aplikacjach.".to_owned(),
...
impl App {
    ...
    fn show_config(&self) {
        ...
        let quality = match parse_quality(&config.segmentation.quality) {
            Ok(bgcam_core::VariantPreference::Fixed(variant)) => self
                .variants
                .iter()
