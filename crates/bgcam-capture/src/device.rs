...
pub fn is_virtual_bgcam_camera(name: &str, symbolic_link: &str) -> bool {
...
pub fn list_cameras() -> Result<Vec<CameraDevice>, CaptureError> {
    ...
    for activate in taken.into_iter().flatten() {
        ...
        if !is_virtual_bgcam_camera(&name, &symbolic_link) {
...
mod tests {
    ...
    fn virtual_camera_is_recognised_by_name_or_clsid() {
        assert!(is_virtual_bgcam_camera("bgcam", r"\\?\SWD#VCAMDEVAPI#something"));
        assert!(is_virtual_bgcam_camera(
            "renamed",
            r"\\?\SWD#VCAMDEVAPI#{4525794b-703e-444b-a81d-2b278b2ad0e4}#x"
        ));
        assert!(!is_virtual_bgcam_camera("USB Camera", r"\\?\USB#VID_046D&PID_085C#1"));
    }
