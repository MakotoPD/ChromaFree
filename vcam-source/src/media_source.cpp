...
namespace
{
    ...
    constexpr OutputMode DefaultMode{ 1280, 720, 30, 1 };
}

HRESULT MediaSource::Initialize(IMFAttributes* attributes, DWORD sessionId)
{
    if (attributes)
    {
    ...
    auto channel = std::make_shared<FrameChannel>(sessionId);
    const auto mode = channel->Open() ? channel->Mode().value_or(DefaultMode) : DefaultMode;
    RETURN_IF_FAILED(_stream->Initialize(this, StreamId, std::move(channel), mode));
