...
use chromafree_ipc::layout::CHROMAFREE_FRAME_CAPACITY;
use chromafree_ipc::{
...
const USAGE: &str = "usage:
  chromafree-probe produce [--size WxH] [--fps N] [--format nv12|bgra] [--seconds S]
  chromafree-probe read [--format nv12|bgra] [--seconds S]
  chromafree-probe camera";
...
fn read(options: &Options) -> Result<()> {
    ...
    let mut buffer = vec![0u8; CHROMAFREE_FRAME_CAPACITY as usize];
